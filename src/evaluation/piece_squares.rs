...
pub fn score_piece_squares(board: &Board, phase: i32) -> i32 {
    let mg_score = score_piece_squares_phase(board, &MG_TABLE);
    let eg_score = score_piece_squares_phase(board, &EG_TABLE);
    let score = interp_phase(mg_score, eg_score, phase);
    if board.us == BLACK {
        -score
    } else {
        score
...
where F: Fn(u64, &[i32; 64]) -> i32 {
    let mut score = f(ps.get_pawns(), &table[PAWN as usize]);
    score += f(ps.get_knights(), &table[KNIGHT as usize]);
    score += f(ps.get_bishops(), &table[BISHOP as usize]);
    score += f(ps.get_rooks(), &table[ROOK as usize]);
    score += f(ps.get_queens(), &table[QUEEN as usize]);
    score += f(ps.get_king(), &table[KING as usize]);
    score
}
